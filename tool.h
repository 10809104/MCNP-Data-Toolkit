...
FileInfo get_file_info(const char *path) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        // 尋找關鍵字行，標示數據區塊的開始
        if (strstr(line, "   tally   ")) {
            
            // --- 步驟 A: 統計此行出現幾次 "tally"，代表有幾組數據 (Groups) ---
            char *ptr = line;
            while ((ptr = strstr(ptr, "tally")) != NULL) {
                report.tally_count++;
                ptr += 5; // 跳過關鍵字繼續搜尋
            }
            ...
            if (fgets(line, sizeof(line), fp)) { 
                ...
                // A. 先跳過開頭的 NPS 數值
                if (sscanf(ptr, "%ld%n", &nps_val, &offset) == 1) {
                    ptr += offset;
                }
    ...
    long read_pos = (file_size > 1024) ? (file_size - 1024) : 0;
    fseek(fp, read_pos, SEEK_SET);

    size_t bytes_read = fread(line, 1, 1024, fp);
    line[bytes_read] = '\0';
...
int loadSingleFileData(const char* path, DynamicTable* dt, long long particles, int groups, int set, char **labels) {
    ...
    char target_p[16];
    sprintf(target_p, "%lld", particles);
    
    char **tally_ids = (char **)malloc(groups * sizeof(char *)); 
    if (!tally_ids) {
        fclose(fp);
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strstr(line, "   tally   ") && state == 0) {
            int tally_count = 0;
            char *ptr = line;
            
            while ((ptr = strstr(ptr, "   tally   ")) != NULL && tally_count < groups) {
                ptr += 11; // 跳過關鍵字
        ...
        // 2. 抓取對應粒子數 (NPS) 的數據列
        if (strstr(line, target_p) && state == 1) {
...
void exportFinalReport(const char* folder, DynamicTable* origin, TableSet* myData, char** myFiles) {
    ...
    // --- Row 2: 提取檔名中的數值 (例如從 AP-0.01.o 提取 0.01) ---
    fprintf(fp, ",,"); 
    for (int i = 0; i < myData->file_count; i++) {
        char temp_name[256];
        strcpy(temp_name, myFiles[i]);
        ...
        // 根據該檔案佔用的欄位數補足逗號
        for (int c = 0; c <= myData->tables[i].cols; c++) fprintf(fp, ",");
    }
    fprintf(fp, "\n");
    ...
    // --- Row 4 之後: 對齊數據列 ---
    int data_row_offset = 1; // 數據內容從第 1 列開始 (第 0 列是標頭)
    
    for (int r = 1; r < origin->rows; r++) {
        // 檢查 source 的原始列是否為空，若空則跳過，避免輸出出現斷層
        const char *cell0 = origin->table[r][0].data.s;
        const char *cell1 = origin->table[r][1].data.s;
        
        if ((cell0 == NULL || strlen(cell0) == 0) && (cell1 == NULL || strlen(cell1) == 0)) {
            continue; 
        }

        // 1. 輸出最左側的兩欄 (來自 source.csv)
        ...
        for (int f = 0; f < myData->file_count; f++) {
            ...
            if (data_row_offset < dt->rows) {
                for (int c = 0; c < dt->cols; c++) {
                    fprintf(fp, ",");
                    if (dt->table[data_row_offset][c].type == TYPE_STRING) {
                        fprintf(fp, "%s", dt->table[data_row_offset][c].data.s ? dt->table[data_row_offset][c].data.s : "");
                    } else {
                        fprintf(fp, "%g", dt->table[data_row_offset][c].data.d); 
                    }
                }
                fprintf(fp, ","); // 檔案間隔空欄
            } else {
                // 若該檔案數據已印完，則補足空位
                for (int c = 0; c <= dt->cols; c++) fprintf(fp, ",");
            }
        }
        fprintf(fp, "\n");
        data_row_offset++; // 僅在成功印出有效行時遞增偏移量
    }
